...
# Name: driver.py
#
# - Spawns Ray workers to runs sim 
# - Performs RL training via SAC
# - Adapted from: https://github.com/marmotlab/ARiADNE
###############################################################################
...
ray.init()
print("Welcome to Search-TTA!")

log_dir = os.path.join(TRAIN_PATH, datetime.now().strftime('%b%d_%H-%M-%S') + '_' + socket.gethostname())  
...
def writeToTensorBoard(writer, tensorboardData, curr_episode):
    ...
    reward, value, policyLoss, qValueLoss, entropy, policyGradNorm, qValueGradNorm, log_alpha, alphaLoss, travel_dist, success_rate, explored_rate, targets_found = tensorboardData
    ...
    writer.add_scalar(tag='Perf/Targets Found', scalar_value=targets_found, global_step=curr_episode)
...
def main():
    ...
    log_alpha = torch.FloatTensor([LOG_ALPHA_START]).to(device)
    ...
    ### Launch meta agents ###
    if GPU_RESOURCE_REQUEST is not None:
        meta_agents = []
        for meta_agent_id, gpu_req in enumerate(GPU_RESOURCE_REQUEST):
            runner = RLRunner.options(num_gpus=gpu_req).remote(meta_agent_id)
            meta_agents.append(runner)
    else:
    ...
    metric_name = ['travel_dist', 'success_rate', 'explored_rate', 'targets_found']
    training_data = []
    perf_metrics = {}
    ...
    try:
        while True:
            ...
            weights_set.append(q_net1_weights)
             ...
             ### Hard Q updates to get target Q-networks every 64 steps ###
            ...
            if curr_episode % 32 == 0:
                ...
                path_checkpoint = "./" + MODEL_PATH + "/" + MODEL_NAME
